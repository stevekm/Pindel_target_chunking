...
Channel.fromPath( file(params.targetsBed) ).into{ targets_bed;
    targets_bed2;
    targets_bed3;
    targets_bed4 }
...
process pindel_noChunk {
    tag "${prefix}"
    publishDir "${params.outputDir}/noChunk/raw", mode: 'copy'
    ...
    output:
    file("${output_dir}")
    set val("${label}"), val(comparisonID), val(tumorID), val(normalID), file("${output_vcf}") into variants_noChunk
...
process normalize_vcfs_noChunk {
    publishDir "${params.outputDir}/noChunk/normalized", mode: 'copy'

    input:
    set val(label), val(comparisonID), val(tumorID), val(normalID), file(vcf), file(ref_fasta), file(ref_fai), file(ref_dict) from variants_noChunk.combine(ref_fasta9).combine(ref_fai9).combine(ref_dict9)

    output:
    set val(label), val(comparisonID), val(tumorID), val(normalID), file("${norm_vcf}") into norm_vcfs_noChunk
    ...
    "${norm_vcf}"
    """
    // bcftools norm --fasta-ref "${ref_fasta}" --output-type v - | \ // this messes up the output for some reason
}
...
process filter_vcf_noChunk {
    ...
    input:
    set val(label), val(comparisonID), val(tumorID), val(normalID), file(vcf) from norm_vcfs_noChunk
...
process pindel_chromChunk {
    tag "${prefix}"
    publishDir "${params.outputDir}/chromChunk/raw", mode: 'copy'
    ...
    output:
    file("${output_dir}")
    set val("${label}"), val(chrom), val(comparisonID), val(tumorID), val(normalID), file("${output_vcf}") into variants_chromChunk
...
process normalize_vcfs_chromChunk {
    publishDir "${params.outputDir}/noChunk/normalized", mode: 'copy'

    input:
    set val(label), val(chrom), val(comparisonID), val(tumorID), val(normalID), file(vcf), file(ref_fasta), file(ref_fai), file(ref_dict) from variants_chromChunk.combine(ref_fasta10).combine(ref_fai10).combine(ref_dict10)

    output:
    set val(label), val(chrom), val(comparisonID), val(tumorID), val(normalID), file("${norm_vcf}") into norm_vcfs_chromChunk
...
process filter_vcf_chromChunk {
    ...
    input:
    set val(label), val(chrom), val(comparisonID), val(tumorID), val(normalID), file(vcf) from norm_vcfs_chromChunk
...
process pindel_nChunk {
    tag "${prefix}"
    publishDir "${params.outputDir}/${chunkLabel}/raw", mode: 'copy'
    ...
    output:
    file("${output_dir}")
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file("${output_vcf}") into variants_nChunk
...
process normalize_vcfs_nChunk {
    publishDir "${params.outputDir}/${chunkLabel}/normalized", mode: 'copy'

    input:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file(vcf), file(ref_fasta), file(ref_fai), file(ref_dict) from variants_nChunk.combine(ref_fasta11).combine(ref_fai11).combine(ref_dict11)

    output:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file("${norm_vcf}") into norm_vcfs_nChunk
...
process filter_vcf_nChunk {
    // filter the .vcf to only include 'PASS' entries
    publishDir "${params.outputDir}/${chunkLabel}/filtered", mode: 'copy', overwrite: true

    input:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file(vcf) from norm_vcfs_nChunk
...
process pindel_lineChunk {
    ...
    output:
    file("${output_dir}")
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file("${output_vcf}") into variants_lineChunk
...
process normalize_vcfs_lineChunk {
    publishDir "${params.outputDir}/${chunkLabel}/normalized", mode: 'copy'

    input:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file(vcf), file(ref_fasta), file(ref_fai), file(ref_dict) from variants_lineChunk.combine(ref_fasta12).combine(ref_fai12).combine(ref_dict12)

    output:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file("${norm_vcf}") into norm_vcfs_lineChunk
...
process filter_vcf_lineChunk {
    // filter the .vcf to only include 'PASS' entries
    tag "${prefix}"
    publishDir "${params.outputDir}/${chunkLabel}/filtered", mode: 'copy'

    input:
    set val(chunkLabel), val(targetChunkNum), val(comparisonID), val(tumorID), val(normalID), file(vcf) from norm_vcfs_lineChunk
...
vcf_tsv_noChunk_refactor.mix(vcf_tsv_chromChunk_refactor, vcf_tsv_nChunk_refactor, vcf_tsv_lineChunk_refactor)
...
merged_tables.collectFile(name: "Pindel.chunking_validation.annotations.tsv", storeDir: "${params.outputDir}", keepHeader: true, sort: false)
